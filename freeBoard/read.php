...
<?php
    ...
    if ($c_user == $login_name || $login_id == 'admin' || $_SESSION['access'] || $c_lock_flag==0) {
        ...
        <?php
            if ($total_reply) {
                ...
                while ($row = mysqli_fetch_array($result)) {
                    ...
                    if ($r_lock_flag && ($r_user != $login_name && $login_id != 'admin' && $c_user != $login_name)) {
                        echo "
                            <div class='reply_class'>
                                <div class='reply_content_area'>
                                    $r_user ";
                        if ($r_user == $login_name || $login_id == 'admin') {
                        ...
                        echo "
                                    <p class='reply_content'>$r_content</p>
                                    <p class='reply_date'>$r_date <a onclick='reply_reply($cnt)'>답글 쓰기</a></p>
                                </div>
                        ";
                    }
...
<?php
        ...
        if ($_POST['submit_reply']) {
            ...
            if (isset($_POST['locked'])) {
                $query = "INSERT INTO FreeBoard_Reply(con_no, r_group, r_class, r_user, r_content, r_lock_flag) VALUES ($con_no, $group, 0, '$login_name', '".$_POST['content']."', 1)";
                mysqli_query($conn, $query);
            } else {
                $query = "INSERT INTO FreeBoard_Reply(con_no, r_group, r_class, r_user, r_content) VALUES ($con_no, $group, 0, '$login_name', '".$_POST['content']."')";
                mysqli_query($conn, $query);
            }
            ...
            if (isset($_POST['locked'])) {
                $query = "INSERT INTO FreeBoard_Reply(con_no, r_group, r_class, r_user, r_content, r_lock_flag) VALUES ($con_no, ".$_POST['group'].", ".$_POST['class'].", '$login_name', '".$_POST['content']."', 1)";
                mysqli_query($conn, $query);
            } else {
                $query = "INSERT INTO FreeBoard_Reply(con_no, r_group, r_class, r_user, r_content) VALUES ($con_no, ".$_POST['group'].", ".$_POST['class'].", '$login_name', '".$_POST['content']."')";
                mysqli_query($conn, $query);
            }
            echo "<script>window.location.href = location.href;</script>";
            ...
            if (isset($_POST['locked'])) {
                $query = "UPDATE FreeBoard_Reply SET r_content=\"".$_POST['content']."\", r_lock_flag=1 WHERE r_no=$r_no";
                mysqli_query($conn, $query);
            } else {
                $query = "UPDATE FreeBoard_Reply SET r_content=\"".$_POST['content']."\", r_lock_flag=0 WHERE r_no=$r_no";
                mysqli_query($conn, $query);
            }
            echo "<script>window.location.href = location.href;</script>";
        } //POST MODIFY REPLY
